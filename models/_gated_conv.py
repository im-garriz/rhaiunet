...
class GatedConv2d(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
        ...
        super(GatedConv2d, self).__init__(in_channels, out_channels, kernel_size, stride,
        ...
        if kernel_size == 1:
            ...
            self.ci = nn.Linear(self.g, out_channels //
                                (in_channels // self.g), bias=False)
            ...
            self.gd2 = nn.Linear(
                self.num_lat, kernel_size * kernel_size, False)
    ...
    def forward(self, x):
        ...
        if self.ind:
            ...
            if self.g > 3:
                ...
                oc = self.ci(self.act(self.ci_bn2(ce2).transpose(2, 1))).transpose(
                    2, 1).contiguous()
            ...
            oc = self.gd2(oc)
            # produce gate
            out = self.sig(out.view(b, 1, c, self.ks, self.ks) +
                           oc.view(b, self.oc, 1, self.ks, self.ks))

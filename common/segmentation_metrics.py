...
def get_evaluation_metrics(logger, epoch, dataloader, segmentor, DEVICE, writer=None, SAVE_SEGS=False, COLOR=True,
    ...
    with open(os.path.join(folder, 'metrics.csv'), 'w') as file:
        file.write('image_id,ccr,precision,recall,specififty,f1_score,jaccard,dsc,roc_auc,pr_auc,usr,osr,hausdorff_error\n')
    ...
    with torch.no_grad():
        ...
        for i, batched_sample in enumerate(dataloader):
            ...
            images, masks, filenames, images_orig = batched_sample["image"].to(DEVICE), batched_sample["mask"].to(DEVICE), \
                                                    batched_sample["filename"], batched_sample["image_orig"].to(DEVICE)
            ...
            for j in range(images.shape[0]):
                ...
                jaccard_coef, f1_score = compute_jaccard_dice_coeffs(segmentation.numpy(), mask.numpy())
                dice_coeff = f1_score
                ...
                hausdorf_error_ = compute_hausdorff_dist(segmentation.numpy(), mask.numpy())
                hausdorf_error = metrics.hausdorff_distance(segmentation.numpy(), mask.numpy())
                ...
                with open(os.path.join(folder, 'metrics.csv'), 'a') as file:
                    file.write(f'{name},{ccr},{precision},{recall},{specifity},{f1_score},{jaccard_coef},{dice_coeff},{roc_auc},{precision_recall_auc},{usr},{osr},{hausdorf_error}\n')
...
def get_evaluation_metrics_sections(logger, epoch, dataloader, segmentor, DEVICE, writer=None, SAVE_SEGS=False, COLOR=True,
    ...
    with torch.no_grad():
        ...
        for i, batched_sample in enumerate(dataloader):
            ...
            for j in range(images.shape[0]):
                ...
                with open(os.path.join(folder, filename_per_point), 'a') as file:
                    file.write(f'{f1_score},{jaccard_coef},{usr},{osr},{hausdorf_error},{area}\n')
        ...
        f1_scores_s1 = np.array(f1_scores_s1)[~np.isnan(np.array(f1_scores_s1))]
        f1_scores_s2 = np.array(f1_scores_s2)[~np.isnan(np.array(f1_scores_s2))]
        f1_scores_s3 = np.array(f1_scores_s3)[~np.isnan(np.array(f1_scores_s3))]
        f1_scores_s4 = np.array(f1_scores_s4)[~np.isnan(np.array(f1_scores_s4))]
        f1_scores_s5 = np.array(f1_scores_s5)[~np.isnan(np.array(f1_scores_s5))]
        
        dice_coeffs_s1 = np.array(dice_coeffs_s1)[~np.isnan(np.array(dice_coeffs_s1))]
        dice_coeffs_s2 = np.array(dice_coeffs_s2)[~np.isnan(np.array(dice_coeffs_s2))]
        dice_coeffs_s3 = np.array(dice_coeffs_s3)[~np.isnan(np.array(dice_coeffs_s3))]
        dice_coeffs_s4 = np.array(dice_coeffs_s4)[~np.isnan(np.array(dice_coeffs_s4))]
        dice_coeffs_s5 = np.array(dice_coeffs_s5)[~np.isnan(np.array(dice_coeffs_s5))]
    ...
    with open(os.path.join(folder, filename), 'w') as file:
        ...
        file.write(f"F1 score s1: {mean_f1_s1:.4f} +- {std_f1_s1:.3f} ({len(f1_scores_s1)})\n")
        file.write(f"F1 score s2: {mean_f1_s2:.4f} +- {std_f1_s2:.3f} ({len(f1_scores_s2)})\n")
        file.write(f"F1 score s3: {mean_f1_s3:.4f} +- {std_f1_s3:.3f} ({len(f1_scores_s3)})\n")
        file.write(f"F1 score s4: {mean_f1_s4:.4f} +- {std_f1_s4:.3f} ({len(f1_scores_s4)})\n")
        file.write(f"F1 score s5: {mean_f1_s5:.4f} +- {std_f1_s5:.3f} ({len(f1_scores_s5)})\n\n")

        file.write(f"JC s1: {mean_dsc_s1:.4f} +- {std_dsc_s1:.3f} ({len(dice_coeffs_s1)})\n")
        file.write(f"JC s2: {mean_dsc_s2:.4f} +- {std_dsc_s2:.3f} ({len(dice_coeffs_s2)})\n")
        file.write(f"JC s3: {mean_dsc_s3:.4f} +- {std_dsc_s3:.3f} ({len(dice_coeffs_s3)})\n")
        file.write(f"JC s4: {mean_dsc_s4:.4f} +- {std_dsc_s4:.3f} ({len(dice_coeffs_s4)})\n")
        file.write(f"JC s5: {mean_dsc_s5:.4f} +- {std_dsc_s5:.3f} ({len(dice_coeffs_s5)})\n\n")
        
        file.write(f"USR s1: {mean_usr_s1:.4f} +- {std_usr_s1:.3f} ({len(usr_s1)})\n")
        file.write(f"USR s2: {mean_usr_s2:.4f} +- {std_usr_s2:.3f} ({len(usr_s2)})\n")
        file.write(f"USR s3: {mean_usr_s3:.4f} +- {std_usr_s3:.3f} ({len(usr_s3)})\n")
        file.write(f"USR s4: {mean_usr_s4:.4f} +- {std_usr_s4:.3f} ({len(usr_s4)})\n")
        file.write(f"USR s5: {mean_usr_s5:.4f} +- {std_usr_s5:.3f} ({len(usr_s5)})\n\n")
        
        file.write(f"OSR s1: {mean_osr_s1:.4f} +- {std_osr_s1:.3f} ({len(osr_s1)})\n")
        file.write(f"OSR s2: {mean_osr_s2:.4f} +- {std_osr_s2:.3f} ({len(osr_s2)})\n")
        file.write(f"OSR s3: {mean_osr_s3:.4f} +- {std_osr_s3:.3f} ({len(osr_s3)})\n")
        file.write(f"OSR s4: {mean_osr_s4:.4f} +- {std_osr_s4:.3f} ({len(osr_s4)})\n")
        file.write(f"OSR s5: {mean_osr_s5:.4f} +- {std_osr_s5:.3f} ({len(osr_s5)})\n\n")
        
        file.write(f"HD s1: {mean_hd_s1:.4f} +- {std_hd_s1:.3f} ({len(hd_s1)})\n")
        file.write(f"HD s2: {mean_hd_s2:.4f} +- {std_hd_s2:.3f} ({len(hd_s2)})\n")
        file.write(f"HD s3: {mean_hd_s3:.4f} +- {std_hd_s3:.3f} ({len(hd_s3)})\n")
        file.write(f"HD s4: {mean_hd_s4:.4f} +- {std_hd_s4:.3f} ({len(hd_s4)})\n")
        file.write(f"HD s5: {mean_hd_s5:.4f} +- {std_hd_s5:.3f} ({len(hd_s5)})\n")
